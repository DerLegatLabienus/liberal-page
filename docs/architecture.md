...
| Data | Postgres via Neon/Docker — Drizzle ORM + `node-postgres`; pglite in tests. Tables grouped into 6 domain schemas — `parliament` / `auth` / `email` / `letters` / `analytics` / `config` (see `data-schema.md`). `src/data/*.json` for static content only |
...
- **`server/services/r2-client.ts`** — S3-compatible client constructed per call from `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, and `R2_BUCKET`. When any env var is unset, `isConfigured()` returns false and routes return `503 R2 not configured`; the client itself throws `R2NotConfiguredError` if called while unconfigured (there is no null singleton).
- **`letter_media_assets` table** — stores `id`, `key`, `filename`, `content_type`, `size_bytes`, `uploaded_by`, and `created_at`. The **public URL is derived on read** (`R2_PUBLIC_BASE_URL + '/' + key`) and never stored in the DB, so updating the public origin requires no data migration.
...
Published letters get a standalone, no-login R2-hosted page for sharing and one-click sending, gated by the `publicSharePages` feature flag (dark-merged off; on in prod).

- **`server/services/share-renderer.ts`** — `renderShareHtml(view, opts)` builds a standalone RTL HTML document (Open Graph + Twitter Card meta, sanitized body, mailto/Gmail send links, a copy-to-clipboard control) for `letter/{id}.html`. `buildOgCardNode(view)` builds a 1200x630 branded card (wordmark + title + CTA) rendered to PNG via `satori` + `@resvg/resvg-js` (both lazy-imported so a native-binding failure can't take down server boot) for `letter/{id}.png`. Since satori has no bidi support, Hebrew strings are pre-reordered to visual order with `server/services/bidi.ts` (`bidi-js`) before being handed to satori. When `TURNSTILE_SITE_KEY` is configured, the page renders as an **interstitial**: the letter (`#letter-content`) is hidden until a Cloudflare Turnstile *Managed* widget passes (client-side reveal via the `tsSolved` callback; `tsError` unhides a fallback link to the main-site letter page). The live token is then read (`turnstile.getResponse()`) and sent as each beacon's body for server-side verification.
- **`server/services/share-publisher.ts`** — publishes/removes the per-letter `.html` + `.png` pair in the same R2 bucket as letter media images; called fire-and-forget from `admin-letters` on letter create/update/delete/publish-status change. Never throws (failures are logged, not surfaced) so a share-publish hiccup can't block the admin letter save.
- **The send page itself is a public surface**: no login, no member-only gating — anyone with the link gets mailto/Gmail one-click send buttons and a copy-body control against the letter's `to`/`cc`/`bcc` addresses, built with the same `src/lib/letter-urls.ts` builders used by the authenticated member flow.
- **Tracking:** the send page's buttons call `navigator.sendBeacon` to `POST /api/public/letters/:id/send?action=mailto|gmail|copy` (`server/routes/public-letters.ts`). No auth; mounted with a permissive `cors()` *before* the app's allowlisted CORS middleware so cross-origin beacons from the R2-served page aren't blocked. Fire-and-forget: always responds `204`, records only when `lettersEnabled` is on and the letter is `published`, and is throttled per `(ip, letter, action)` for 10s to absorb double-fires. Writes go to dedicated `public_mailto` / `public_gmail` / `public_copy` analytics buckets (`server/repositories/letter-analytics-repository.ts`), kept separate from the authenticated member `mailto`/`copy` buckets so public reach and member engagement don't conflate. The admin letters dashboard (`src/pages/AdminLettersPage.tsx`, `src/lib/letter-sends.ts`) shows the public vs. member split per letter. When the `publicSendTurnstile` flag is on, a send is recorded only after the beacon's Cloudflare Turnstile token (carried in the request body as text/plain) verifies server-side via `server/services/turnstile.ts` (`verifyTurnstile` → Cloudflare `siteverify`): `rejected` tokens are dropped, while a missing `TURNSTILE_SECRET_KEY` fails open (`skip` → still counted) so a misconfig can't silently zero the metric. Flag off = no verification (current behavior).
- **Config:** `APP_PUBLIC_URL` (site origin for the page's "join & send" CTA link) and `API_PUBLIC_URL` (backend origin the tracking beacon targets) are both optional env vars with production defaults (GitHub Pages URL / `https://liberal-page.onrender.com`) — see `.env.example`.
- **Multi-channel:** since the 2026-07-15 channels feature, the share page sources its content from the letter's `letter_channels` (via `attachChannels` + `buildChannelSends` in `share-publisher.ts`), not the legacy flat columns. It renders the email block (subject/body/mailto/Gmail as before) **plus** a per-recipient link list for each SMS/WhatsApp channel — `sms:`/`wa.me` deep links (WhatsApp links open in a new tab) each firing a `?action=sms|whatsapp&contactId=N` beacon (new `public_sms`/`public_whatsapp` buckets, also rolled into `lifetime`). See `data-schema.md#multi-channel-letters-letter_channels`.
