...
async function main() {
  ...
  // Default letter address-book contacts (ministries + baseline MKs) are seeded by
  // migration 0020_seed_letter_contacts.sql, which runs automatically on startup —
  // not here — so the address book is populated even on a from-scratch boot without
  // `npm run db:seed`. See server/db/migrations/0020_seed_letter_contacts.sql.
