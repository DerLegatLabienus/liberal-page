...
- **`recentRanking: "progress"`** — re-rank the Recent tab by the most recent *genuine legislative event* per bill (committee session appearances), not creation order. **OData schema verified (2026-06-13):** `KNS_CmtSessionItem` links via `ItemID` (= BillID) when `ItemTypeID eq 2` — there is no `BillID` field. `CommitteeSessionID` is auto-incremented and usable as a recency proxy (no second date-fetch needed). Spec at `docs/superpowers/specs/2026-06-13-bills-overview-phase2-design.md` — blocked on 3 user decisions (include plenum? pool size? show/hide bills with no sessions?). Do NOT use `KNS_Bill.LastUpdatedDate` (administrative-only; surfaced old bills as "recent" in a prior incident).
