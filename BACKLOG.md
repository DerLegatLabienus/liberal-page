...
`oklch(...)` values, so every shadcn colour utility became `hsl(oklch(...))` — invalid CSS,
dropped by the browser, rendering transparent site-wide.

**Fix:** the variables now hold **bare oklch components** (`--primary: 0.546 0.245 262.881`)
and the config references them as `oklch(var(--primary) / <alpha-value>)`. That form is what
makes Tailwind's opacity modifiers work — the earlier naive attempt (`var(--x)`) broke the
build on `@apply outline-ring/50` (`src/index.css:85`) precisely because a bare `var()` can't
take injected alpha.

Two special cases handled: `--destructive-foreground` was an HSL triplet (converted to oklch),
and the dormant `.dark` block had alpha-baked values (`oklch(1 0 0 / 10%)`) which became solid
— dark mode is never activated in code. `--popover`/`--chart-*`/`--sidebar-*` were left as full
`oklch()` since the config doesn't map them.

Verified in a browser via the CSSOM: `bg-primary`, `bg-card`, `bg-muted`, `border-border`,
`text-muted-foreground` and the `oklch(var(--primary) / .9)` alpha form all compute to real
colours. **Follow-up:** `src/components/layout/AuthControl.tsx` still hardcodes `blue-600` as a
workaround from when the tokens were dead — it can now go back to `bg-primary`.
